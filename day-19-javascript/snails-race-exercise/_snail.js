...
class Snail {
  ...
  render() {
    ...
    const snailBody = this.element.querySelector('.snail__shell');
    snailBody.style.fill = `${this.color}`;
  ...
  update() {
    ...
    const run = setInterval(() => {
      ...
      distance += speed;
      this.element.style.marginLeft = `${distance}vw`;
      console.log(distance);
      console.log(this.element.style.marginLeft);

      if (distance >= 95) {
        clearInterval(run);

        const resultEl = document.querySelector('.race__result');
        resultEl.textContent = `Finished!`
      }
    }, 250);
  }
