...
/* document.addEventListener('DOMContentLoaded', () => {
  ...
  const chatOutputEl = document.querySelector('.chat__bubbles');

  // iterate over array and with every array item, print LI with side as class, name and text;

  conversation.forEach(element => {
    if (element.side === 'remote') {
      chatOutputEl.innerHTML += `
      <div class = "chat__wrapper ${element.side}">
        <img src="cleverbot.png">
        <li class="">${element.text}</li>
      </div>`;
    } else {
      chatOutputEl.innerHTML += `
      <div class = "chat__wrapper ${element.side}">
